...
const ProjectAssessmentHub: React.FC<{ onLogin?: (email: string) => void }> = ({ onLogin }) => {
  ...
  const [summaryTab, setSummaryTab] = useState<'telemetry' | 'advisory' | 'vault'>('telemetry');
  
  const [submission, setSubmission] = useState({
    intent: '',
    urgency: 'standard' as 'standard' | 'expedited' | 'critical',
    visualization: { type: 'visualization' as const, format: 'link' as const, value: '' },
    spec: { type: 'spec' as const, format: 'link' as const, value: '' },
    linkedin: '',
    phone: '',
    email: '',
    country: 'US',
  ...
  const handleNext = async () => {
    const val = validateStep(currentStep, submission);
    ...
    if (currentStep === 'identity') {
      ...
      if (genericDomains.includes(domain)) {
        setErrors({ email: "[!] REJECTED: Institutional C3 requires a private business domain email node." });
        return;
      }
    }
    ...
    if (!val.isValid) {
      ...
      Object.keys(val.errors || {}).forEach(k => {
        stringifiedErrors[k] = String(val.errors[k]);
      });
      setErrors(stringifiedErrors);
      setBlockingReasons(val.blockingReasons || []);
      return;
    }
  ...
  const handleVerifyCode = async (code: string) => {
    ...
    if (String(code) === '123456') {
      setSubmission(prev => ({ 
        ...prev, 
        otpVerified: true, 
        signals: { ...prev.signals, otp_verified: true } 
      }));
      
      setIsTransitioning(true);
      await new Promise(r => setTimeout(r, 600));
      setCurrentStep('privy_handshake');
      setIsTransitioning(false);
      setIsLoading(false);
    } else {
      setOtpError(true);
  ...
  const executePrivyHandshake = async () => {
    setIsLoading(true);
