...
import { 
  Database, Cpu, Zap, ArrowUpRight, Activity, Globe, 
  ChevronUp, ChevronDown, ChevronLeft, ChevronRight, 
  ShieldCheck, FileText, Target, Briefcase, Network, ShieldAlert, Coins, Package, Terminal,
  Workflow, Layers, Shield, Code, HardDrive, Share2
} from 'lucide-react';
...
const ProjectCard = memo(({ node }: { node: ProjectNode }) => {
  ...
  return (
    <div 
      ...
      className="flex-shrink-0 w-[340px] sm:w-[500px] lg:w-[580px] snap-center group bg-[#020617] border border-white/5 rounded-[3rem] p-8 lg:p-12 flex flex-col transition-all duration-700 ease-expo relative overflow-hidden transform-gpu hover:border-decensat/40 hover:shadow-2xl h-[680px]"
    >
      <div className="absolute inset-0 z-0 transition-transform duration-700 ease-out pointer-events-none"
        style={{ transform: `scale(1.1) translate(${mousePos.x * -15}px, ${mousePos.y * -15}px)` }}>
        <div className="mt-7 absolute inset-0">
          <Image
            src={node.image}
            alt={node.name}
            fill
      ...
      <div className="relative z-10 flex-1 flex flex-col">
        <div className="flex justify-between items-start mb-8">
          <div className="px-4 py-1.5 border border-white/10 bg-white/5 rounded-lg text-[9px] font-mono text-slate-500 uppercase tracking-widest group-hover:text-decensat group-hover:border-decensat/30 transition-all">
            RECORD_{node.id.replace('-', '_').toUpperCase()}
          </div>
          <div className="px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-[0.2em] bg-decensat text-black shadow-glow-sm">
            {node.year}
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="text-[10px] font-bold text-slate-600 uppercase tracking-[0.4em] flex items-center gap-3">
             <Activity size={12} className="text-decensat animate-pulse" /> {node.systemType}
          </div>
        </div>
        ...
        <div className="mb-8 flex items-center gap-4">
            <button 
              ...
              className="flex items-center gap-2 text-[10px] font-black text-decensat uppercase tracking-[0.3em] hover:text-white transition-all group/btn outline-none rounded-sm"
            >
              {isExpanded ? '[TERMINATE_VIEW]' : '[EXPAND_DETAILS]'} {isExpanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
            </button>
        </div>

        <div className={`transition-all duration-700 ease-expo overflow-hidden ${isExpanded ? 'max-h-[300px] opacity-100 mb-8' : 'max-h-0 opacity-0'}`}>
             <div className="grid grid-cols-2 gap-8 p-6 bg-white/5 border border-white/10 rounded-2xl relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none"><Terminal size={40} /></div>
                <div>
                   <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-3">Technical Stack</div>
                   <div className="flex flex-wrap gap-2">{node.techStack.map(s => (<span key={s} className="text-[8px] font-mono text-white bg-black/40 px-2 py-1 rounded border border-white/5 uppercase">{s}</span>))}</div>
                </div>
                <div className="text-right flex flex-col justify-end">
                   <div className="text-[9px] font-black text-slate-600 uppercase tracking-widest mb-1">Partner</div>
                   <div className="text-sm font-black text-white uppercase tracking-tighter italic">{node.clientPartner}</div>
                </div>
             </div>
        </div>
    ...
    <div className="space-y-16">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 border-l-[12px] border-decensat pl-12 animate-in slide-in-from-left duration-1000">
        ...
        <div className="flex gap-4 shrink-0 pb-2">
           <button 
              onClick={() => scroll('left')} 
              className="p-4 rounded-2xl bg-white/5 border border-white/10 text-slate-500 hover:text-decensat hover:border-decensat transition-all active:scale-90 shadow-xl group"
      ...
      <div 
        ref={scrollRef}
        className="flex gap-10 overflow-x-auto snap-x snap-mandatory no-scrollbar pb-20 custom-scrollbar-h"
      >
